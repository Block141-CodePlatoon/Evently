...
class GuestList(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            return Response({"result": f"Guest id {guest.id} created"})
    ...
    def put(self, request, pk):
        ...
        if serializer.is_valid(raise_exception=True):
            guest = serializer.save()
            return Response({"result": f"Guest id {guest.id} updated"})

    def delete(self, request, pk):
        guest = get_object_or_404(Guest.objects.all(), pk=pk)
        guest.delete()
        return Response({"result": f"Guest id {pk} deleted"}, status=204)
